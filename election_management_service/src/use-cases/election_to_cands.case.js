const Candidate = require("../core/models/candidates.model")
...
module.exports = class ElectionsUseCase {
    ...
    async findAllCandidates(election_id, query) {

        const clause = {
            where: { election_id, ...query },
    ...
    async findAllInvolvedElections(candidate_id) {

        const elections = await this.electionToCandsRepo.find({ where: { candidate_id } });

        // Use Promise.all to map asynchronously over the array
        const res = await Promise.all(
            elections.map(async (el) => {
                return await this.electionsRepo.findOne({ where: { election_id: el.election_id } });
            })
        );
        return res;
